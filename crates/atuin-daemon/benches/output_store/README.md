...
The record-count and year scenarios use `Store::open`, which explicitly disables Atuin's wall-clock flush and GC tasks for controlled replay. Both backends also support the shared wall-clock maintenance policy through `Store::open_with_limit`, described below. Commands occur in simulated bursts of five within a five-second window, followed by a longer idle period. Every burst, day and checkpoint ends with a durable commit. Fjall uses `persist(SyncAll)`; redb uses non-durable write transactions followed by an immediate commit that also persists preceding transactions. Partial bursts are flushed too. This prevents machine speed from deciding how many captures share a durability interval. Fjall's own background flushing and compaction remain enabled, so checkpoint layout and size can vary with scheduling. Repeat runs when investigating small differences.

Retention evicts the same oldest records from both stores. The requested reclamation amount is the sum of their serialized-value sizes, matching Fjall's eviction-candidate selection followed by removal. This is **not physical disk-budget enforcement**: a fixed physical limit would retain different records in each engine. Budget-driven GC is available separately, but is not mixed into these fixed-retention scenarios.
...
The same physical budget need not retain the same records in the two engines. Use the controlled scenarios for equal-data space comparisons, and `open_with_limit` when investigating wall-clock durability and budget-driven retention. The replay executable does not wait out simulated days or enable budget-driven eviction.
...
The console prints a table; `report.json` contains exact byte counts, corpus statistics, workload parameters, generated/live record counts and activity mixes, reuse counts, verified-capture counts and per-file sizes. Each checkpoint persists and closes the database before walking its files, preventing races with background file deletion. It then reopens and verifies every retained capture.
