...
pub async fn start_server(path: &str) -> (String, oneshot::Sender<()>, JoinHandle<()>, tempfile::NamedTempFile) {
    ...
    let temp_db = tempfile::NamedTempFile::with_suffix("-atuin-server-sqlite-test.db").unwrap();
    let db_uri =
        env::var("ATUIN_DB_URI").unwrap_or_else(|_| format!("sqlite://{}", temp_db.path().display()));
    ...
    (format!("http://{addr}{path}"), shutdown_tx, server, temp_db)
}
